...
- (void)readData {
    ...
    if (bytesRead > 0) {
        ...
        if ( [self validatePingResponsePacket:packet sequenceNumber:&sequenceNumber] ) {
            ...
            if ((strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(simplePing:didReceivePingResponsePacket:timeElasped:)] ) {
                [strongDelegate simplePing:self didReceivePingResponsePacket:packet timeElasped:timeInterval];
            }
        } else {
